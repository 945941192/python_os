...
summary_process_html()
{
    ...
    local summary_process_field_html="       
         <table class=\"table table-bordered\" name=\"summary_cpu_process_table\"  style=\"word-break:break-all; word-wrap:break-all;\">
            <thead>
                <tr class=\"tr_top\" height=\"30px\">
                  <th width=\"10%\" style=\"text-align: center;vertical-align: middle;\">${PID}</th>
                  <th width=\"10%\" style=\"text-align: center;vertical-align: middle;\">${COUNT}<div>(点击可排序)</div></th>
    ...
    do
        ...
        local summary_process_line_html+="
                <tr class=\"text-center\" height=\"25px\">
                    <td height=\"25px\">${pid}</td>
                    <td height=\"25px\">${num}</td>
                    <td height=\"25px\">${wide_wchan_column}</td>
                    <td height=\"25px\">${commands}</td>
                </tr>"
...
function summary_cpu_process_check_info(){ 
    var oTable = document.getElementById("show_data"); 
    for(var i=0;i<oTable.rows.length;i++){ 
        if(i%2==0){     
        oTable.rows[i].className = "even_line"; 
        }
...
window.onload=function(){
    ...
    var summary_cpu_process_table=document.getElementsByName("summary_cpu_process_table")[0];
    makeSortable(check_report_table);
    makeSortable(summary_cpu_process_table);
